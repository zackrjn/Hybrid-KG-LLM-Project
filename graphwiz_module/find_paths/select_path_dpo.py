...
def align_json_files(task_dicts: Dict, folder_path: str, task: str):
    ...
    for key, value in aligned_dict.items():
        # select the longest as the CoT_response
        ...
        longest = ""   
        if len(aligned_dict[key]['pos_response']) > 0:
            for s in aligned_dict[key]['pos_response']:
                if len(longest) < len(s):
                    longest = s
            aligned_dict[key]["CoT_response"] = longest
    return aligned_dict
...
def CoT_response(task):
    ...
    # v2
    with open('/home/yuhanli/Graph-Reasoning-LLM/datasets/data/graph_v2_dsformat.json') as f:
        datas = f.readlines()
    
    task_dicts = {}
    for data in datas:
        data = json.loads(data)
        start_index = data['prompt'].find("### Instruction:\n") + len("### Instruction:\n")
        end_index = data['prompt'].find("\n\n### Response:")
        query = data['query'][start_index: end_index].replace('\n', '')
        if task == data['task']:
            task_dict = {
                "CoT_response": data["chosen"],
                "answer": data["response"],
                'task': data['task'],
                'pos_response': [],
                'neg_response': []
...
def main():
    ...
    if os.path.exists(Path(args.save_path) / f"{args.task}_rft_dpo_dicts.json"):
        ...
        task_dicts = {}
        folder_path = "/home/yuhanli/Graph-Reasoning-LLM/datasets/dpo_data"
